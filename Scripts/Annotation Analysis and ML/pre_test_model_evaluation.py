...
def get_optimal_model_params(dataset,sent,model_type,opimizer_grid_search=False):
    ...
    compiled_model =KerasRegressor(model= create_model_for_grid_dense, dropout_rate = 0.6,activation="tanh",weight_constraint=1.0, input_shape = X.shape[1:],optimizer_grid_search=opimizer_grid_search)
    grid = GridSearchCV(estimator=compiled_model,n_jobs=-1,cv=split_data_using_cross_validation(original_dataset,sent), param_grid=param_grid,scoring="neg_root_mean_squared_error",error_score='raise')
...
# def extract_grid_params(grid_params):
