...
# gets text grid + timestamps, returns csv file for each episode with row for each segment, the columns are start ene text
textGrid_input_dir = ''  # textGrid files
timeStamps_input_dir = ''  # json files with segments time stamp
output_segments_dir = ''   # saving time stamped segments
...
if __name__ == '__main__':
    try:
        for filename in os.listdir(textGrid_input_dir):
            ...
            timeStamps_path = os.path.join(timeStamps_input_dir, f'{file}.json') # assuming json files has the same name as textGrid files
